	...
	case "$SCRIPT_LANG" in
		zh)
			case "$key" in
				...
				macos_satisfied) echo "✅ macOS 版本符合要求 (需要 macOS 13+)" ;;
				macos_unsatisfied) echo "❌ macOS 版本不符合要求 (需要 macOS 13+，当前: $1)" ;;
		...
		ja)
			case "$key" in
				...
				macos_satisfied) echo "✅ macOS バージョンが要件を満たしています (macOS 13+ が必要)" ;;
				macos_unsatisfied) echo "❌ macOS バージョンが要件を満たしていません (macOS 13+ が必要、現在: $1)" ;;
		...
		en|*)
			case "$key" in
				...
				macos_satisfied) echo "✅ macOS version meets requirements (macOS 13+ required)" ;;
				macos_unsatisfied) echo "❌ macOS version does not meet requirements (macOS 13+ required, current: $1)" ;;
...
echo "  macOS 版本: $MACOS_VERSION $([ "$MACOS_VERSION" != "未知" ] && [ "$(echo "$MACOS_VERSION" | cut -d'.' -f1)" -ge 13 ] 2>/dev/null && echo '✅ 符合要求' || echo '⚠️  需要 macOS 13+')"
